...
const resident = computed(() => users.value.find(user => user.role === 'resident'))
...
const residentName = computed(() => resident.value?.name ?? 'Morador')
...
<template>
  <div class="app-shell">
    ...
    <main class="main-content">
      <AppTopbar :name="residentName" :initials="residentInitials" :compact="isScrolled" @notifications="showNotifications = !showNotifications" />
      ...
      <div class="page-wrap">
        <section class="page-heading">
          <div><div class="eyebrow">Segunda-feira, 14 de setembro de 2024 · Residencial Vale Verde</div><h1>Olá, {{ residentName }}!</h1><p>Veja o que está acontecendo no seu condomínio hoje.</p></div>
        ...
        <div v-else>
          <section class="section-intro"><div class="section-icon"><SvgIcon :name="active === 'Reservas' ? 'calendar' : active === 'Ocorrências' ? 'alert' : 'message'" /></div><div><h2>{{ active }}</h2><p>{{ active === 'Reservas' ? 'Agende e acompanhe os espaços do condomínio.' : active === 'Ocorrências' ? 'Registre solicitações e acompanhe cada atendimento.' : 'Informação importante para viver melhor em comunidade.' }}</p></div><button v-if="active === 'Ocorrências'" class="primary-button" @click="showOccurrence = true">Nova ocorrência</button></section>
          ...
          <article class="panel detail-panel" v-else><div v-if="active === 'Ocorrências' && !recentOccurrences.length" class="empty-row">Nenhuma ocorrência registrada.</div><div v-if="active === 'Comunicados' && !recentNotices.length" class="empty-row">Nenhum comunicado publicado.</div><button v-for="item in (active === 'Comunicados' ? recentNotices : recentOccurrences)" :key="item.id" class="detail-row interactive-row" @click="openDetail(active === 'Comunicados' ? 'notice' : 'occurrence', item.id)"><span class="row-icon large"><SvgIcon :name="active === 'Comunicados' ? 'message' : 'alert'" /></span><span><strong>{{ item.title }}</strong><small>{{ active === 'Comunicados' ? noticeMeta(item.publishedAt, item.status) : `${item.category} · ${formatDateTime(item.createdAt)}` }}</small></span><em :class="active === 'Comunicados' ? noticeTone(item.status) : occurrenceTone(item.status)">{{ active === 'Comunicados' ? noticeLabel(item.status) : occurrenceLabel(item.status) }}</em><span class="arrow"></span></button></article>
        </div>
      </div>
    </main>
